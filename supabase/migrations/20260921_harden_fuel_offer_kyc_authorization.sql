-- CargoDek fuel authorization hardening
-- Enforces active fuel roles and verified buyer KYC inside SECURITY DEFINER RPCs.

create or replace function public.cd_create_fuel_offer(
  p_enquiry_id uuid,
  p_price numeric,
  p_currency text,
  p_quantity numeric,
...
begin
  ...
  select * into v_enquiry from public.fuel_enquiries where id=p_enquiry_id for update;
  ...
  if not public.cd_is_member(v_listing.supplier_company_id) then raise exception 'Not a member of the supplier company'; end if;
  if not public.cd_has_role(v_listing.supplier_company_id,'fuel_supplier'::public.business_role) then raise exception 'Fuel supplier role is not active'; end if;
  if v_enquiry.status<>'pending' then raise exception 'Fuel enquiry is no longer pending'; end if;
  if p_price<=0 or p_quantity<=0 then raise exception 'Price and quantity must be greater than zero'; end if;
  if p_quantity>v_enquiry.quantity or p_quantity>v_listing.quantity then raise exception 'Offer quantity exceeds available quantity'; end if;
  insert into public.fuel_offers(enquiry_id,supplier_company_id,price,currency,quantity,payment_terms,delivery_terms,message)
  values(p_enquiry_id,v_listing.supplier_company_id,p_price,upper(trim(p_currency)),p_quantity,p_payment_terms,p_delivery_terms,p_message)
...
begin
  ...
  if v_offer.quantity<=0 or v_offer.quantity>v_enquiry.quantity_requested then raise exception 'Offer quantity is invalid'; end if;
